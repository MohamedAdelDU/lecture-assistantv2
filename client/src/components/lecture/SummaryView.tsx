...
export function SummaryView({ summary, title }: SummaryViewProps) {
  ...
  const parseStructuredSummary = (fullSummary: string, lang: string): ParsedSections | null => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Check for section headings (exact match or starts with heading)
      if (trimmed === introHeading || trimmed.startsWith(introHeading + " ")) {
        currentSection = "intro";
        continue;
      } else if (trimmed === summaryHeading || trimmed.startsWith(summaryHeading + " ")) {
        currentSection = "summary";
        continue;
      } else if (trimmed === keyPointsHeading || trimmed.startsWith(keyPointsHeading + " ")) {
        ...
        <div style="font-family: 'Tajawal', Arial, sans-serif; direction: ${dir}; color: ${textColor}; line-height: 1.8; padding: 20px; background-color: ${bgColor};">
          <div style="text-align: center; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 3px solid ${primaryColor};">
            ...
            <p style="font-size: 9px; color: ${mutedText}; margin-top: 10px;">
              ${contentLang === "ar" ? "تم التصدير بواسطة Lecture Assistant" : "Exported by Lecture Assistant"} • ${new Date().toLocaleDateString(contentLang === "ar" ? "ar-EG" : "en-US")}
            </p>
          </div>
          <div style="margin-bottom: 20px;">
            <h2 style="font-size: 18px; font-weight: bold; color: ${primaryColor}; border-bottom: 2px solid ${primaryColor}; padding-bottom: 5px; margin-bottom: 10px; text-align: ${textAlign};">
          ...
          <div style="margin-top: 30px; padding-top: 15px; border-top: 2px solid ${borderColor}; text-align: center;">
            <p style="font-size: 9px; color: ${mutedText};">
              ${contentLang === "ar" 
                ? "© 2025 Lecture Assistant. جميع الحقوق محفوظة. هذا المستند محمي بحقوق النشر ولا يجوز نسخه أو توزيعه دون إذن." 
                : "© 2025 Lecture Assistant. All rights reserved. This document is protected by copyright and may not be copied or distributed without permission."}
            </p>
          </div>
        </div>
      `;
  ...
  return (
    <div className="space-y-6" dir={displayDir} key={`summary-${language}-${detectContentLanguage}`}>
      {/* Header */}
